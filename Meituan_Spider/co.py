...
def get_cookie():
    ...
    while mark == 0:
        ...
        driver = webdriver.Firefox(firefox_binary=fire_path)
        # driver = webdriver.Chrome(executable_path='C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe',
        #                           chrome_options=chrome_options)
        driver.set_page_load_timeout(8)  # 设置超时
        driver.set_script_timeout(8)
        url = 'https://qd.meituan.com/meishi/'  # 美团青岛首页
        url2 = 'http://meishi.meituan.com/i/?ci=60&stid_b=1&cevent=imt%2Fhomepage%2Fcategory1%2F1'  # 美食页面
        try:
            driver.get(url)
            time.sleep(2.5)
